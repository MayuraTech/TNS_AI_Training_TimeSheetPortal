...
export class UsersComponent implements OnInit {
  ...
  async load() {
    ...
    try {
      let url = '/qa/api/admin/users?size=100';
  ...
  async saveUser() {
    ...
    try {
      if (this.editingUser()) {
        ...
        await firstValueFrom(this.http.post('/qa/api/admin/users',
          { fullName: this.form.fullName, email: this.form.email, department: this.form.department, employeeId: this.form.employeeId, roles: this.form.roles },
          { withCredentials: true }
        ));
      }
  ...
  async deactivate(id: number) {
    ...
    try { await firstValueFrom(this.http.post(`/qa/api/admin/users/${id}/deactivate`, {}, { withCredentials: true })); this.load(); } catch { }
  }
  async reactivate(id: number) {
    try { await firstValueFrom(this.http.post(`/qa/api/admin/users/${id}/reactivate`, {}, { withCredentials: true })); this.load(); } catch { }
  }
  async resetPassword(id: number) {
    if (!confirm('Send password reset email?')) return;
    try { await firstValueFrom(this.http.post(`/qa/api/admin/users/${id}/reset-password`, {}, { withCredentials: true })); alert('✅ Reset email sent!'); } catch { }
  }
